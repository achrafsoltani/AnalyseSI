...
class SQLGenerator:
    ...
    def generate(self) -> str:
        ...
        lines = []
        lines.append("-- Generated by Merisio")
    ...
    def _generate_entity_table(self, entity: Entity) -> str:
        ...
        table_name = self._safe_name(entity.name)
        lines = []
        ...
        for attr in entity.attributes:
            col_name = self._get_column_name(entity.name, attr.name)
                    ...
                    if other_link.entity_id != entity.id and other_link.cardinality_max == "N":
                        ...
                        if other_entity:
                            for pk_attr in other_entity.get_primary_keys():
                                # Original FK name before override
                                original_fk_name = f"fk_{self._safe_name(other_entity.name)}_{pk_attr.name}"
                                col_name = self._get_column_name(entity.name, original_fk_name)
                                ref_col = self._get_column_name(other_entity.name, pk_attr.name)
                                col_def = f"    {col_name} {pk_attr.get_sql_type()}"
                                columns.append(col_def)
                                columns.append(
                                    f"    FOREIGN KEY ({col_name}) "
                                    f"REFERENCES {self._safe_name(other_entity.name)}({ref_col})"
                                )
        ...
        for link in links:
            ...
            if entity:
                for pk_attr in entity.get_primary_keys():
                    # Original FK name before override
                    original_fk_name = f"fk_{self._safe_name(entity.name)}_{pk_attr.name}"
                    col_name = self._get_column_name(assoc.name, original_fk_name)
                    ref_col = self._get_column_name(entity.name, pk_attr.name)
                    col_def = f"    {col_name} {pk_attr.get_sql_type()} NOT NULL"
                    columns.append(col_def)
                    pk_columns.append(col_name)
                    fk_constraints.append(
                        f"    FOREIGN KEY ({col_name}) "
                        f"REFERENCES {self._safe_name(entity.name)}({ref_col})"
                    )

        # Add carrying attributes (association's own attributes)
        for attr in assoc.attributes:
            col_name = self._get_column_name(assoc.name, attr.name)
            col_def = f"    {col_name} {attr.get_sql_type()}"
            columns.append(col_def)
    ...
    def _get_column_name(self, table_name: str, original_name: str) -> str:
        """Get the column name, applying any MLD overrides."""
        # Table names in MLD are uppercase
        override = self._project.get_mld_column_name(table_name.upper(), original_name)
